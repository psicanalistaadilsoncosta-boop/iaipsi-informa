...
export default function SaboresPostPage() {
  ...
  return (
    <main style={{ maxWidth: '780px', margin: '0 auto', padding: '30px 20px', fontFamily: 'system-ui, sans-serif', backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      ...
      {/* Receita — só aparece se estiver salva */}
      {recipe && (
        <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '32px' }}>
          <button
            onClick={() => setShowRecipe(!showRecipe)}
            style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 24px', borderRadius: '10px', border: 'none', backgroundColor: '#b45309', color: '#fff', fontWeight: 700, fontSize: '1rem', cursor: 'pointer', marginBottom: '24px' }}>
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '14px', border: '1px solid #e5e7eb', padding: '32px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid #f3f4f6' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>{recipe.fonte}</div>
                  <div style={{ fontSize: '0.68rem', color: '#d1d5db', marginTop: '2px' }}>
                    Receita traduzida e adaptada para o português
                  </div>
                </div>
              </div>
          ...
          <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #f3f4f6', lineHeight: 1.6 }}>
            {recipe.fonte?.includes('TheMealDB')
              ...
              : '📖 Receita tradicional adaptada para o português brasileiro por Adilson Costa. Proporções e técnica baseadas na culinária original do prato.'}
          </p>
        </div>
      )}

      {showRecipe && recipe.error && (
            <p style={{ color: '#dc2626', fontSize: '0.9rem' }}>{recipe.error}</p>
          )}
        </div>
      )}
